...
use api\modules\v4\service\UserToken;
use api\modules\v4\service\Yorder as YorderService;
...
class YorderController extends BaseActiveController
{
    ...
    public function actionList($sellerID){
        $yyxmModels=YyxmModel::find()->where(['user_id'=>$sellerID,'status'=>1])->select(['xiangmu','money','id'])->asArray()->all();
        $yyxmArray=[];
        foreach ($yyxmModels as $item){
            array_push($yyxmArray,$item['xiangmu'].' '.$item['money']);
        }
        return[
            'yyxmModels'=>$yyxmArray
        ];
    }
