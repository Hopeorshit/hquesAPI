...
use api\modules\qyz\controllers\BaseController;
...
class WebAccessToken
{
    ...
    {
        $this->tokenUrl=sprintf(YII::$app->params['qyz']['web_access_token'],Yii::$app->params['qyz']['app_id'],Yii::$app->params['qyz']['app_secret'],$code);
    }
...
//        $cache->set('zp',$token,7200);
    ...
    private function getFromWxServer(){//从服务器读取
      ...
      $token=json_decode($token,true);
        if(isset($token['errmsg'])){//TODO Render Json
         header('Location:'.Yii::$app->params['domain'].'qyz/home/userinfo');
         die();
        }
      return $token;
    }
    ...
    private function getRefreshFromWxServer($refreshToken){
        $senUrl=sprintf(Yii::$app->params['qyz']['refresh_token'],Yii::$app->params['qyz']['app_id'],$refreshToken);
    ...
    private function checkAccessToken($token){
      $senUrl=sprintf(Yii::$app->params['qyz']['check_refresh'],$token['access_token'],$token['openid']);
