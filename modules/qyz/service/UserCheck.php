...
use api\modules\qyz\service\User as UserService;
...
//        }else{
//           self::redirect(Yii::$app->params['domain'].'qyz/home/userinfo');
//           die();
//        }
//     }
    ...
    public static function grandCooike(){//授权cookie返回用户
        ...
        }else
        {
            if(isset($_GET['code']))//微信机制，通过转发打开的会屏蔽掉code码，复制微信内部链接也会屏蔽掉后面的code,来实现必须在微信内部打开链接
            {
                ...
                $sendUrl=sprintf(Yii::$app->params['qyz']['user_info'],$token['access_token'],$token['openid']);
